...
- General playlist imports follow the automatic-grouping preference. Imports inside comparison add versions to the current item. Capture the destination when import begins so navigation cannot redirect an unfinished import.
...
No public API is added. Existing import and transport interfaces are destination-aware and mode-aware. Automatic grouping feeds the same workspace value and atomic import transaction rather than introducing a second import path.
...
- No named projects, multiple windows, crossfade, guaranteed gapless album transitions, metadata editing, or automatic quality-based version selection in v1.
