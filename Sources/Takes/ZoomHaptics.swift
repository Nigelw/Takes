...
enum HapticPatternOption: String, CaseIterable, Identifiable {
    case off
    case alignment
    case levelChange
    case generic
...
@MainActor
final class ZoomHapticsController: ObservableObject {
    private var zoomThresholdBucket: Int?
    private let performer: NSHapticFeedbackPerformer
    private let zoomControlPattern = HapticPatternOption.levelChange
    ...
    func syncZoomProgress(_ progress: Double) {
        zoomThresholdBucket = ZoomHapticTriggerGate.zoomThresholdBucket(for: progress)
    }

    func updateZoomProgress(_ progress: Double) {
        let bucket = ZoomHapticTriggerGate.zoomThresholdBucket(for: progress)
    ...
    private func perform(_ pattern: HapticPatternOption) {
