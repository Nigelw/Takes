...
/// Like `RealFFT` (AnalysisDSP.swift) but exposes the complex half-spectrum
/// instead of accumulating power — the coherence detector needs phases and
/// the flicker detector reuses the same pass. Same Hann window and
/// normalization: a full-scale sine's bin power reads ~0 dB.
private final class ComplexSpectrumFFT {
    let size: Int
